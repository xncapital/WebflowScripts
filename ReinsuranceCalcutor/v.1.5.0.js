$(function () {
    ...
    const controller = {
        ...
        adjustSliderAndInputs(responsibilityPercentage) {
            let reinsurerPercentage = 100 - responsibilityPercentage;
            $('#quota-share--responsibility').val(responsibilityPercentage + '%');
            $('#quota-share--reinsurer').val(reinsurerPercentage + '%');
    ...
    const renderGraph = (isXolActive, isAggActive, isQuotaActive, xol_max_limit, xol_ap, agg_ap, aggPayouts_array, insured_percentage, insurer_percentage) => {
        ...
        dotXolAp.find('.reinsurance--graph--text').text(`${formatLargeNumber(xol_ap)} XoL Attachment Point`)
        dotAggAp.find('.reinsurance--graph--text').text(`${formatLargeNumber(agg_ap)} Agg Attachment Point`)
        sideLabelAp.text(`${formatLargeNumber(xol_ap)} XoL Attachment Point`)
        ...
        if (!isXolActive && !isAggActive && !isQuotaActive) {
            ...
            $('.reinsurance--slider--card.loss').find('.reinsurance--input').each((index, element) => {
                ...
                if (totalLoss <= agg_ap) {
                    ...
                    if (totalLoss !== agg_ap) dotAggAp.find('.reinsurance--graph--text').text(`${formatLargeNumber(agg_ap)} Agg Attachment Point (%${Math.round((agg_ap - totalLoss) / agg_ap * 100)} left)`)
                    else dotAggAp.find('.reinsurance--graph--text').text(`${formatLargeNumber(agg_ap)} Agg Attachment Point`)
    ...
    const handleCalculations = () => {
        ...
        let quotaPercentage = parseToNumber($('#quota-share--reinsurer').val());
        let insurerPercentage = parseToNumber($('#quota-share--responsibility').val());
        ...
        if (!isXolActive && !isAggActive && !isQuotaActive) {
            ...
            $('.reinsurance--slider--card.loss').find('.reinsurance--input').each((index, element) => {
                ...
                if (currentLoss - XoL_AP < 0) {
                    ...
                    XoL_Payout = XoL_Payout + (XoL_MaxLimit - XoL_AP);
                    let current_Payout = XoL_MaxLimit - XoL_AP
                    currentQuota_payout = (currentLoss - current_Payout) * quotaPercentage
                    newBalance = newBalance - (currentLoss - current_Payout) * insurerPercentage;
                } else {
                    // Loss exceeds AP but within Max Limit, payout up to Loss - AP
                    let payout = currentLoss - XoL_AP;
                    XoL_Payout += payout;
                    currentQuota_payout = XoL_AP * quotaPercentage
                    newBalance = newBalance - XoL_AP * insurerPercentage
                }
        ...
        renderGraph(isXolActive, isAggActive, isQuotaActive, XoL_MaxLimit, XoL_AP, agg_AP, aggPayouts_array)
