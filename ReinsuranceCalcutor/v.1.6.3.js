$(function () {
    ...
    const handleFilters = () => {
        ...
        $('#max-limit--input--label').text('XoL Max<br>(per occurrence)')
        ...
        if (!isXolActive && !isAggActive && !isQuotaActive) {
            ...
            $('#max-limit--input--label').text('Quota Max<br>(per occurrence)')
            $('.reinsurance--summary--card.xol-payout').hide();
    ...
    const renderGraph = (isXolActive, isAggActive, isQuotaActive, xol_max_limit, xol_ap, agg_ap, aggPayouts_array, insured_percentage, insurer_percentage, aggCheckpointWidth) => {
        ...
        const blockStyle = (options) => {
            ...
            if (type === 'base') {
                ...
                let formatted_agg_payout = formatLargeNumber(aggPayouts_array[aggBlockIndex])
                $newblock.show();
                $newblock.attr('id', `agg-${aggBlockIndex}`)
                ...
                $newblock.find(text_MiddleRight).find('.reinsurance--graph--text').text(`${formatted_agg_payout}`);
        ...
        if (!isXolActive && !isAggActive && !isQuotaActive) {
            ...
            sideLabelAp.text(`${formatLargeNumber(xol_ap)} Quota AP`)
            ...
            $('.reinsurance--slider--card.loss').find('.reinsurance--input').each((index, element) => {
                ...
                if (totalLoss <= agg_ap) {
                    ...
                    if (totalLoss - currentLoss >= agg_ap) {
                        aggBlockWidth = aggCheckpointWidth
                        if (aggBlockWidth === 0) aggBlockWidth = 84
                        relativePosition = aggCheckpointWidth + (index) * 84;
                        console.log(`index: ${index} - aggBlockWidth: ${aggBlockWidth} - relativePosition: ${relativePosition}`)
                    }
                    if (aggBlockWidth < 84) $newBlock_agg.css('border-top-left-radius', '0px')
                    dotAggAp.find('.reinsurance--graph--text').text(`${formatLargeNumber(agg_ap)} Agg AP`)

                    //if (index === 0 && agg_ap <= xol_ap) return

                    if (index === numberOfLosses - 1) {
                        console.log('aggPayouts_array[index]', aggPayouts_array[index])
                        if (aggPayouts_array[index] > 0) {
                            blockStyle({ type: 'agg', $newblock: $newBlock_agg, isLast: true, aggPosition: relativePosition, aggBlockWidth, aggBlockIndex: index, isAggLast: true, currentLoss })
                        }
                    } else {
                        if (aggPayouts_array[index] > 0) blockStyle({ type: 'agg', $newblock: $newBlock_agg, isLast: false, aggPosition: relativePosition, aggBlockWidth, aggBlockIndex: index, isAggLast: false, currentLoss })
                    }
    ...
    const handleCalculations = () => {
            ...
            $('.reinsurance--slider--card.loss').find('.reinsurance--input').each((index, element) => {
                ...
                if (currentLoss <= XoL_AP) {
                    ...
                    if (index < aggCheckpoint) {
                        ...
                        agg_current_payout = totalLoss - XoL_Payout - agg_AP
                        aggCheckpointWidth = 84 * agg_current_payout / currentLoss;

                    } else {
                        agg_current_payout = currentLoss
                    }
                } else if (currentLoss > XoL_AP && currentLoss <= XoL_MaxLimit) {
                    ...
                    if (index < aggCheckpoint) {
                        ...
                        agg_current_payout = totalLoss - XoL_Payout - agg_AP
                        if (agg_current_payout <= 0) agg_current_payout = 0
                        aggCheckpointWidth = 84 * agg_current_payout / XoL_AP;
                    } else {
                    ...
                    if (index < aggCheckpoint) {
                        ...
                        if (XoL_AP - agg_AP > 0) {
                            agg_current_payout = XoL_AP - agg_AP
                            aggCheckpointWidth = 84 * agg_current_payout / XoL_AP;
                        }
                        else agg_current_payout = 0
        ...
        renderGraph(isXolActive, isAggActive, isQuotaActive, XoL_MaxLimit, XoL_AP, agg_AP, aggPayouts_array, insured_percentage, insurer_percentage, aggCheckpointWidth)
