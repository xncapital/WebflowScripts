$(function () {
    ...
    let firstLossRowTemplate = $('.reinsurance--loss--row').eq(0).clone();
    ...
    const renderGraph = (isXolActive, isAggActive, isQuotaActive, xol_max_limit, xol_ap, agg_ap, insured_percentage, insurer_percentage, aggCheckpointWidth
        ...
        sideLabelAp.html(`${formatLargeNumber(xol_ap)}<br> XoL AP`);

        sideLabelMax.html(`${formatLargeNumber(xol_max_limit)} Max`)
        ...
        if (!isXolActive && !isAggActive && !isQuotaActive) {
            ...
            baseBlocks.each((index, element) => {
                ...
                if (currentLoss === 0) {
                    ...
                    //console.log(captivePayouts_array[index])
                    ...
                    if (aggPayouts_array[index] > 0) {
                        ...
                        $(aggBlocks).eq(index).css('max-height', `${maxHeight}px`);
                        ...
                        if (index > aggCheckpoint) {
                            ...
                            //console.log('entered')
                            $(aggBlocks).eq(index).find('.reinsurance--block--value').css('font-size', `10px`);
                            $(aggBlocks).eq(index).css('border-top-left-radius', `0px`);
                            $(element).find('.reinsurance--block--value').css('font-size', `10px`);
            ...
            relativePosition = 0
            let $targetBlock = $('.reinsurance--graph--block.agg').filter(function () {
                return $(this).css('display') === 'flex';
            }).first();

            if ($targetBlock.length > 0) {
                //console.log('Found the block:', $targetBlock);
                //console.log('relativePosition', $targetBlock.parent().position().left)
                if ($targetBlock.width() < 84) {
                    relativePosition = $targetBlock.parent().position().left + 42
                } else {
                    relativePosition = $targetBlock.parent().position().left - 4
                }
            } else {
                relativePosition = 92 * baseBlocks.length
            }
            ...
            baseBlocks.each((index, element) => {
                ...
                if (currentLoss === 0) {
                    ...
                    if (currentLoss > xol_ap) {
                        $(element).css('height', `${xol_ap * usdPixelUnit_y}px`);
                        $(element).find('.reinsurance--block--value').text(`-${formatLargeNumber(xol_ap)}`)
                    } else if (currentLoss < xol_ap) {
                        $(element).css('height', `${(captivePayouts_array[index]) * usdPixelUnit_y}px`);
                        $(element).find('.reinsurance--block--value').text(`-${formatLargeNumber(captivePayouts_array[index])}`)
                    } else if (currentLoss === xol_ap) {
                        $(element).css('height', `${xol_ap * usdPixelUnit_y}px`);
                    ...
                    if (xolPayouts_array[index] > 0) {
                        ...
                        if (currentLoss > xol_max_limit) {
                            $(xolBlocks).eq(index).css('height', `${(xol_max_limit) * usdPixelUnit_y}px`);
                        }
                        $(xolBlocks).eq(index).find('.reinsurance--block--value').css('position', 'absolute')
                        $(xolBlocks).eq(index).find('.reinsurance--block--value').css('top', `${xolPayouts_array[index] * usdPixelUnit_y * 0.5 - 6}px`)
                    }
                    ...
                    if (aggPayouts_array[index] > 0) {
                        $(aggBlocks).eq(index).css('display', 'flex')
                        $(aggBlocks).eq(index).css('height', `${$(element).height()}px`);
                        if (currentLoss > xol_max_limit) {
                            $(aggBlocks).eq(index).css('height', `${(xol_ap) * usdPixelUnit_y}px`);
                        }
                        $(aggBlocks).eq(index).css('width', `${aggCheckpointWidth}px`);
                        ...
                        if (currentLoss <= xol_max_limit) {
                            if (index > aggCheckpoint) {
                                ...
                                $(aggBlocks).eq(index).css('width', `${aggCheckpointWidth}`);
                                $(element).remove()
                            } else if (index === aggCheckpoint && aggPayouts_array[index] !== xol_ap && captivePayouts_array[index] !== 0) { //partial agg
                                $(aggBlocks).eq(index).find('.reinsurance--block--value').css('font-size', `10px`);
                                $(aggBlocks).eq(index).css('border-top-left-radius', `0px`);
                                $(element).find('.reinsurance--block--value').css('font-size', `10px`);
                                $(element).find('.reinsurance--block--value').css('position', `absolute`);
                            ...
                            if (index > aggCheckpoint) {
                                ...
                                aggCheckpointWidth = 84
                                $(aggBlocks).eq(index).css('width', `${aggCheckpointWidth}`);
                                $(aggBlocks).eq(index).find('.reinsurance--block--value').text(`${formatLargeNumber(xol_ap)}`)
                                $(element).remove()

                                $(baseBlocks_excess).eq(index).css('color', `#635699`);
                                $(baseBlocks_excess).eq(index).css('background-color', `#dad3f8`);
                                $(baseBlocks_excess).eq(index).find('.reinsurance--block--value').text(`${formatLargeNumber(captivePayouts_array_above_max[index])}`)
                            } else if (index === aggCheckpoint) {

                                if (aggPayouts_array[index] === (captivePayouts_array_above_max[index])) {
                                    console.log('complete agg at the top')
                                    //complete top and bottom
                                    $(aggBlocks).eq(index).remove()

                                    $(baseBlocks_excess).eq(index).css('color', `#635699`);
                                    $(baseBlocks_excess).eq(index).css('background-color', `#dad3f8`);
                                    $(baseBlocks_excess).eq(index).find('.reinsurance--block--value').text(`${formatLargeNumber(captivePayouts_array_above_max[index])}`)
                                } else if (aggPayouts_array[index] < (captivePayouts_array_above_max[index])) {
                                    console.log('partial agg at the top')
                                    $(baseBlocks_excess).eq(index).find('.reinsurance--block--value').css('font-size', `10px`);
                                    $(baseBlocks_excess).eq(index).find('.reinsurance--block--value').css('position', `absolute`);
                                    $(baseBlocks_excess).eq(index).find('.reinsurance--block--value').css('left', `2px`);
                                    $(baseBlocks_excess).eq(index).find('.reinsurance--block--value').text(`-${formatLargeNumber(captivePayouts_array_above_max[index] - aggPayouts_array[index])}`)

                                    $(aggBlocks).eq(index).find('.reinsurance--block--value').css('font-size', `10px`);
                                    $(aggBlocks).eq(index).css('border-top-left-radius', `0px`);
                                    aggCheckpointWidth = 42
                                    $(aggBlocks).eq(index).css('width', `${aggCheckpointWidth}`);
                                    $(aggBlocks).eq(index).css('height', `55px`);
                                    $(aggBlocks).eq(index).appendTo($(baseBlocks_excess).eq(index))

                                    $(aggBlocks).eq(index).find('.reinsurance--block--value').text(`${formatLargeNumber(aggPayouts_array[index])}`)
                                } else if (aggPayouts_array[index] > (captivePayouts_array_above_max[index]) && aggPayouts_array[index] < (captivePayouts_array_above_max[index] + xol_ap)) {
                                    console.log('partial agg at the bottom')
                                    ...
                                    $(aggBlocks).eq(index).find('.reinsurance--block--value').text(`${formatLargeNumber(aggPayouts_array[index] - captivePayouts_array_above_max[index])}`)
                                    $(element).find('.reinsurance--block--value').text(`-${formatLargeNumber(xol_ap - (aggPayouts_array[index] - captivePayouts_array_above_max[index]))}`)
                                } else if (aggPayouts_array[index] === (captivePayouts_array_above_max[index] + xol_ap)) {
                                    console.log('complete agg at the bottom')
                                    aggCheckpointWidth = 84
                                    $(aggBlocks).eq(index).css('width', `${aggCheckpointWidth}`);
                                    $(aggBlocks).eq(index).find('.reinsurance--block--value').text(`${formatLargeNumber(xol_ap)}`)
                                    $(element).remove()
            ...
            let $parentBlock = $targetBlock.closest('.reinsurance--blocks')

            if ($targetBlock.length > 0) {
                //console.log('Found the block:', $targetBlock);
                //console.log('relativePosition', $targetBlock.parent().position().left)
                if ($targetBlock.width() < 84) {
                    relativePosition = $parentBlock.position().left + 42
                } else {
                    relativePosition = $parentBlock.position().left - 4
                }
            } else {
                relativePosition = 92 * baseBlocks.length
            }
            ...
            sideLabelAp.css('display', 'flex')
            sideLabelMax.css('display', 'flex')
            $('.quota-share--label').css('display', 'flex')
            $('.quota-share--label').find('.reinsurance--label--text').text(`Quota Share: ${Math.round(insurer_percentage * 100)}%`)
            ...
            baseBlocks.each((index, element) => {
                ...
                if (currentLoss === 0) {
                    ...
                    $(element).css('display', 'flex');
                    $(quotaBlocks).eq(index).css('height', `${(captivePayouts_array[index] + quotaPayouts_array[index]) * usdPixelUnit_y}px`);
                    //console.log($(element).height())
                    $(element).find('.reinsurance--block--value').text(`-${formatLargeNumber(captivePayouts_array[index])}`)
                    $(element).find('.reinsurance--block--value').css('position', 'static')
                    $(element).find('.reinsurance--block--value').addClass('quota')


                    $(quotaBlocks).eq(index).css('display', 'flex')
                    $(element).css('height', `${(quotaPayouts_array[index] + captivePayouts_array[index]) * usdPixelUnit_y * insurer_percentage}px`);
                    $(quotaBlocks).eq(index).find('.reinsurance--block--value').text(formatLargeNumber(quotaPayouts_array[index]))

                    $(quotaBlocks).eq(index).find('.quota-block--spacer').css('height', `${$(element).height()}px`)
                    $(quotaBlocks).eq(index).find('.quota-block--spacer').css('min-height', `${$(element).height()}px`)
                    //console.log($(quotaBlocks).eq(index).height())
            ...
            $('.reinsurance--graph--block.base').each((index, element) => {
                if ($(quotaBlocks).eq(index).height() < 80) {
                    //console.log('less than 80')
                    $(quotaBlocks).eq(index).css('height', '80px')
                    $(element).css('height', `${80 * insurer_percentage}px`)
                    $(quotaBlocks).eq(index).find('.quota-block--spacer').css('height', `${30}px`)
                    $(quotaBlocks).eq(index).find('.quota-block--spacer').css('min-height', `${30}px`)
                }

                if ($(quotaBlocks).eq(index).height() - $(element).height() < 30) {
                    $(element).css('height', `${$(element).height() - 30}px`)
        ...
        if (!isXolActive && !isAggActive && !isQuotaActive) {
            ...
            $('.reinsurance--slider--card.loss').find('.reinsurance--input').each((index, element) => {
                ...
                if (totalLoss <= agg_AP) {
                    ...
                    if (index === 0) { // the first loss has reached agg
                        ...
                        agg_current_payout = currentLoss - agg_AP
                        ...
                        if (aggCheckpoint === index) {
                            ...
                            let Q4 = currentLoss - Q2
                            if (Q4 === 0) Q4 = currentLoss
                ...
                captivePayouts_array.push(currentLoss - agg_current_payout)
            });
            ...
            $('.reinsurance--slider--card.loss').find('.reinsurance--input').each((index, element) => {
                ...
                if (currentLoss > XoL_MaxLimit) {
                    ...
                    captivePayouts_array.push(XoL_MaxLimit * insurer_percentage)
                    quotaPayouts_array.push(XoL_MaxLimit * insured_percentage)
            ...
            $('.reinsurance--slider--card.loss').find('.reinsurance--input').each((index, element) => {
                ...
                totalCaptiveLosses = totalCaptiveLosses + currentLoss - current_xol_payout

                if (totalCaptiveLosses <= agg_AP) {
                    agg_current_payout = 0
                    howMuchLeftToAgg = agg_AP - totalCaptiveLosses
                } else {
                    ...
                    if (index === 0) { // the first loss has reached agg
                        ...
                        if (currentLoss > XoL_MaxLimit) {
                            agg_current_payout = totalCaptiveLosses - agg_AP

                            //checking if Agg starts top or bottom
                            /* 
                              Cases:
                                starts bottom, and only partial [done]
                                starts bottom, and fills top partially
                                starts bottom, and fills top completely
                                starts bottom and fills it completely(agg_current_payout = xol_ap)
                                starts top, and only partial
                                starts top, and fills completely (agg_current_payout = currentLoss - xol_max_limit)

                                the simplest strategy I can think of is to use current agg payout and compare with xol ap
                            */
                            if (agg_current_payout < XoL_AP) {
                        ...
                        if (aggCheckpoint === index) {
                            if (currentLoss > XoL_AP) {
                                ...
                                if (currentLoss > XoL_MaxLimit) agg_current_payout = totalCaptiveLosses - agg_AP

                                if ((totalCaptiveLosses + (currentLoss - current_xol_payout)) <= agg_AP) {
                                    howMuchLeftToAgg = 0
                                    agg_current_payout = 0
                                }
                            ...
                            if (currentLoss > XoL_AP) {
                                ...
                                if (currentLoss > XoL_MaxLimit) agg_current_payout = currentLoss - current_xol_payout
                            } else {
                                agg_current_payout = currentLoss
                            }
                        }
                ...
                //console.log('agg_current_payout', agg_current_payout)
                aggCheckpointWidth = howMuchLeftToAgg / agg_AP * 84
            ...
            $('#agg-bar--text').closest('.agg-bar').css('display', 'flex')
            if (totalCaptiveLosses < agg_AP) {
                $('#agg-bar--text').text(`${formatLargeNumber((totalCaptiveLosses))} of ${formatLargeNumber(agg_AP)}`)
                $('.agg-bar--progress').css('width', `${(totalCaptiveLosses) / agg_AP * 100}%`)
            } else {
                $('#agg-bar--text').text(`${formatLargeNumber((agg_AP))} of ${formatLargeNumber(agg_AP)}`)
                $('.agg-bar--progress').css('width', `${100}%`)
            }
            ...
            $('.reinsurance--slider--card.loss').find('.reinsurance--input').each((index, element) => {
                ...
                if (currentLoss <= XoL_AP) {
                    ...
                    newBalance = newBalance - XoL_AP * insurer_percentage - (currentLoss - XoL_MaxLimit);
    ...
    /* INPUTS HANDLER */
    $('.reinsurance--add--row').addClass('glow-effect');

    $('.reinsurance--apply--button').on('click', (e) => {
        $('.reinsurance--graph--block').remove()
        handleCalculations()
        $('.reinsurance--blocks').each((index, element) => {
            setTimeout(() => {
    ...
    let intervalId = setInterval(() => {
        $('.reinsurance-add-loss--plus-icon').removeClass('active');
        $('.reinsurance--add--row--text').removeClass('active');
        setTimeout(() => {   
            $('.reinsurance-add-loss--plus-icon').addClass('active');
        $('.reinsurance--add--row--text').addClass('active');
        }, 1000);
    }, 2000);

    // Event listener to stop the interval
    $('.reinsurance--add--row').on('click', (e) => {
        $('.reinsurance--add--row').removeClass('glow-effect');
        $('.reinsurance--add--row').css('background-color', 'transparent'); // Set to transparent
    ...
    $('.reinsurance--card--header').on('click', (e) => {
        if ($(e.target).closest('.dashboard--info--wrapper').length > 0) return;
    ...
    //calculate
    //$('.reinsurance--apply--button').eq(1).trigger('click')
    $('.reinsurance--loss--row').remove()


    const isFirstVisit = !localStorage.getItem('visited');
    const timeoutDuration = isFirstVisit ? 4000 : 1000;
    ...
    setTimeout(() => {
        $('.reinsurance-calculator--mask').fadeOut(() => {
            $('.reinsurance-calculator--mask').remove();
        });
    }, timeoutDuration);
